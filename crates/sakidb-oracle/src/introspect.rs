use dashmap::DashMap;
use oracle::sql_type::{Clob, ToSql};
use oracle::Connection as OracleConnection;
use sakidb_core::{
    error::{Result, SakiError},
    types::{
        CheckConstraintInfo, ColumnInfo, CompletionBundle, CompletionColumn, CompletionTable,
        ConnectionId, DatabaseInfo, ErdData, ForeignKeyInfo, FunctionInfo, IndexInfo,
        MaterializedViewInfo, PartitionDetail, PartitionInfo, SchemaInfo, SequenceInfo, TableInfo,
        TriggerInfo, UniqueConstraintInfo, ViewInfo,
    },
};
...
impl OracleIntrospector {
    ...
    async fn execute_query(
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let result_set = conn.query(&query, &bind_params).map_err(|e| {
                SakiError::QueryFailed(format!("Introspection query failed: {}", e))
            })?;
    ...
    pub async fn list_columns(
        ...
        let rows = Self::execute_query(
            conn.clone(),
            query,
            vec![schema.to_uppercase(), table.to_uppercase()],
        )
        .await?;
        ...
        let pk_rows = Self::execute_query(
            conn,
            pk_query,
            vec![schema.to_uppercase(), table.to_uppercase()],
        )
        .await?;
        for row in pk_rows {
            let pk_col: String = row
                .get(0)
                .map_err(|e| SakiError::QueryFailed(format!("Failed to get pk col: {}", e)))?;
            if let Some(col) = columns
                .iter_mut()
                .find(|c| c.name.to_uppercase() == pk_col.to_uppercase())
            {
    ...
    pub async fn list_views(&self, conn_id: &ConnectionId, schema: &str) -> Result<Vec<ViewInfo>> {
        let conn = self.get_connection(conn_id)?;
        let query =
            "SELECT view_name, read_only FROM all_views WHERE owner = :1 ORDER BY view_name"
                .to_string();
        let rows = Self::execute_query(conn, query, vec![schema.to_uppercase()]).await?;
    ...
    pub async fn list_functions(
        ...
        for row in rows {
            let name: String = row.get(0).map_err(|e| {
                SakiError::QueryFailed(format!("Failed to get function name: {}", e))
            })?;
            let kind: String = row.get(1).map_err(|e| {
                SakiError::QueryFailed(format!("Failed to get function kind: {}", e))
            })?;
    ...
    pub async fn list_sequences(
        ...
        for row in rows {
            let name: String = row.get(0).map_err(|e| {
                SakiError::QueryFailed(format!("Failed to get sequence name: {}", e))
            })?;
    ...
    pub async fn list_indexes(
        ...
        let pk_query =
            "SELECT index_name FROM all_constraints WHERE owner = :1 AND constraint_type = 'P'"
                .to_string();
    ...
    pub async fn list_triggers(
        ...
        for row in rows {
            let name: String = row.get(0).map_err(|e| {
                SakiError::QueryFailed(format!("Failed to get trigger name: {}", e))
            })?;
            let table_name: String = row.get(1).map_err(|e| {
                SakiError::QueryFailed(format!("Failed to get trigger table: {}", e))
            })?;
            let event: String = row.get(2).map_err(|e| {
                SakiError::QueryFailed(format!("Failed to get trigger event: {}", e))
            })?;
            let tt: String = row.get(3).map_err(|e| {
                SakiError::QueryFailed(format!("Failed to get trigger type: {}", e))
            })?;
            let condition: Option<String> = row.get(4).ok().flatten();
            let function_name: String = row.get::<_, String>(5).unwrap_or_default();
            let status: String = row.get(6).map_err(|e| {
                SakiError::QueryFailed(format!("Failed to get trigger status: {}", e))
            })?;
    ...
    pub async fn list_check_constraints(
        ...
        for row in rows {
            let constraint_name: String = row.get(0).map_err(|e| {
                SakiError::QueryFailed(format!("Failed to get constraint name: {}", e))
            })?;
    ...
    pub async fn list_unique_constraints(
        ...
        for row in rows {
            let constraint_name: String = row.get(0).map_err(|e| {
                SakiError::QueryFailed(format!("Failed to get unique constraint name: {}", e))
            })?;
            let columns: String = row.get(1).unwrap_or_default();
    ...
    pub async fn get_partition_info(
        ...
        let detail_query = "SELECT partition_name, high_value FROM all_tab_partitions
            ...
            .to_string();
        let detail_rows = Self::execute_query(
            conn,
            detail_query,
            vec![schema.to_uppercase(), table.to_uppercase()],
        )
    ...
    pub async fn get_create_table_sql(
        ...
        let rows = Self::execute_query(
            conn,
            query,
            vec![table.to_uppercase(), schema.to_uppercase()],
        )
        .await?;
        if rows.is_empty() {
    ...
    pub async fn get_erd_data(&self, conn_id: &ConnectionId, schema: &str) -> Result<ErdData> {
        ...
        let col_query = "SELECT table_name, column_name, data_type, nullable, data_default
            ...
            .to_string();
        let col_rows =
            Self::execute_query(conn.clone(), col_query, vec![schema.to_uppercase()]).await?;
        ...
        let pk_rows =
            Self::execute_query(conn.clone(), pk_query, vec![schema.to_uppercase()]).await?;
    ...
    pub async fn get_completion_bundle(
        ...
        let mut completion_tables: Vec<CompletionTable> = tables
            .into_iter()
            .map(|t| CompletionTable {
                name: t.name,
                kind: "table".to_string(),
            })
            .collect();
        for view in views {
            completion_tables.push(CompletionTable {
                name: view.name,
                kind: "view".to_string(),
            });
        }
            ...
            Err(_) => {
                ...
                match row.get::<_, Option<Clob>>(idx) {
                    Ok(Some(mut clob)) => {
                        ...
                        use std::io::Read;
                        clob.read_to_string(&mut s).map_err(|e| {
                            SakiError::QueryFailed(format!("Failed to read CLOB: {}", e))
                        })?;
                        Ok(s)
                    }
                    _ => Ok(String::new()),
