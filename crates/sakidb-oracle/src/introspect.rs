...
impl OracleIntrospector {
    ...
    pub async fn get_erd_data(&self, conn_id: &ConnectionId, schema: &str) -> Result<ErdData> {
        ...
        for table in &tables {
            let cols = self.list_columns(conn_id, schema, &table.name).await?;
            columns.insert(table.name.clone(), cols);
            let fks = self.list_foreign_keys(conn_id, schema, &table.name).await?;
            if !fks.is_empty() {
    ...
    pub async fn get_schema_completion_data(&self, conn_id: &ConnectionId, schema: &str) -> Result<HashMap<String, Vec<String>>> {
        ...
        for table in &tables {
            let cols = self.list_columns(conn_id, schema, &table.name).await?;
            let col_names: Vec<String> = cols.into_iter().map(|c| c.name).collect();
            result.insert(table.name.clone(), col_names);
        }
        Ok(result)
    }
