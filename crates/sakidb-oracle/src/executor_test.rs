...
mod tests {
    ...
    fn test_convert_oracle_type_to_string() {
        assert_eq!(
            OracleExecutor::convert_oracle_type_to_string(&OracleType::Varchar2(100)),
            "VARCHAR2"
        );
        assert_eq!(
            OracleExecutor::convert_oracle_type_to_string(&OracleType::Number(10, 2)),
            "NUMBER"
        );
        assert_eq!(
            OracleExecutor::convert_oracle_type_to_string(&OracleType::Date),
            "DATE"
        );
        assert_eq!(
            OracleExecutor::convert_oracle_type_to_string(&OracleType::Timestamp(6)),
            "TIMESTAMP"
        );
    }
    ...
    fn test_is_query_detection() {
        ...
        assert!(OracleExecutor::is_query(
